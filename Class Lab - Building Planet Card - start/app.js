...
// 1: Create a 'Planet' component that renders a planet card (DONE)


// 2: Create a container component that iterates over the planets array 
//    and renders a 'Planet' component for each object in the array (DONE)


// 3: Render the container component to the DOM (DONE)

// 4: Add a DELETE (or "X") Button to each card that removes the corresponding card if clicked (DONE)
...
const AppIndex = () => {
  const [list, setList] = React.useState(planets);
  ...
  return <Container planets={list} onDelete={handleRemove} />;
};

const Container = ({ planets, onDelete }) => (
  <div className="container">
    {planets.map((item) => (
      <Planet key={item.id} props={item} onDelete={onDelete} />
    ))}
  </div>
);

const Planet = ({ props, onDelete }) => (
  <div className="card">
